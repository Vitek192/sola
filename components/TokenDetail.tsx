...
export const TokenDetail: React.FC<Props> = ({ token, onUpdateToken, onRemoveToken, onBack, aiConfig, onNotify }) => {
  ...
  const formatPrice = (price: number) => {
    ...
    if (price < 0.0000000001) return `$${price.toExponential(4)}`;
            ...
            <div className="bg-gray-850 p-6 rounded-xl border border-gray-750 shadow-lg">
                <div className="flex justify-between items-start mb-4">
                    <div>
                        ...
                        <div className="flex gap-2 mt-2">
                             <a href={`https://dexscreener.com/solana/${token.address}`} target="_blank" rel="noreferrer" className="text-xs bg-gray-700 hover:bg-gray-600 text-gray-200 px-2 py-1 rounded flex items-center gap-1">🦅 DexScreener</a>
                             <a href={`https://www.geckoterminal.com/solana/pools/${token.address}`} target="_blank" rel="noreferrer" className="text-xs bg-gray-700 hover:bg-gray-600 text-gray-200 px-2 py-1 rounded flex items-center gap-1">🦎 GeckoTerminal</a>
                             <a href={`https://solscan.io/token/${token.address}`} target="_blank" rel="noreferrer" className="text-xs bg-gray-700 hover:bg-gray-600 text-gray-200 px-2 py-1 rounded flex items-center gap-1">🔍 Solscan</a>
                        </div>
                    </div>
                    
                    <div className="flex bg-gray-800 rounded-lg p-1">
                        <button onClick={() => setShowGrowthChart(false)} className={`px-3 py-1 text-xs font-bold rounded ${!showGrowthChart ? 'bg-gray-600 text-white' : 'text-gray-400 hover:text-white'}`}>Price ($)</button>
                        <button onClick={() => setShowGrowthChart(true)} className={`px-3 py-1 text-xs font-bold rounded ${showGrowthChart ? 'bg-blue-600 text-white' : 'text-gray-400 hover:text-white'}`}>Growth (%)</button>
                    </div>
                </div>
                ...
                <div className="h-[350px] w-full relative">
                    ...
                    <AreaChart data={showGrowthChart ? growthData : token.history}>
                        ...
                        <XAxis dataKey="timestamp" tickFormatter={formatTime} stroke="#718096" minTickGap={30} />
                        <YAxis domain={['auto', 'auto']} stroke="#718096" tickFormatter={showGrowthChart ? (val) => `${val.toFixed(0)}%` : formatPrice} width={85} />
                        <Tooltip contentStyle={{ backgroundColor: '#1a202c', border: '1px solid #2d3748', borderRadius: '8px' }} labelFormatter={(label) => new Date(label).toLocaleString()} formatter={(value: number) => [showGrowthChart ? `${value.toFixed(2)}%` : formatPrice(value), showGrowthChart ? 'Growth' : 'Price USD']} />
                        <Area type="monotone" dataKey={showGrowthChart ? "growth" : "price"} stroke={showGrowthChart ? "#3b82f6" : "#14F195"} strokeWidth={2} fill="url(#colorPrice)" />
                    </AreaChart>
                    </ResponsiveContainer>
                </div>
            </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm text-gray-400">
                        ...
                        <tbody className="divide-y divide-gray-800">
                            {dailyLogs.length === 0 ? (
                                <tr><td colSpan={5} className="px-4 py-3 text-center italic">Not enough data for daily logs yet (Need > 24h)</td></tr>
                            ) : (
                                dailyLogs.map((log) => (
                                    <tr key={log.day} className="hover:bg-gray-800/30">
                                        ...
                                        <td className="px-4 py-3"><span className={`px-2 py-0.5 rounded text-xs font-bold ${log.change >= 0 ? 'bg-green-900/30 text-green-400' : 'bg-red-900/30 text-red-400'}`}>{log.change > 0 ? '+' : ''}{log.change.toFixed(2)}%</span></td>
                                        <td className="px-4 py-3">${(log.volume/1000).toFixed(1)}k</td>
                                        <td className="px-4 py-3"><span className={log.makerChange > 0 ? 'text-green-400' : 'text-gray-500'}>{log.makerChange > 0 ? '+' : ''}{log.makerChange}</span></td>
                                    </tr>
                                ))
                            )}
                        </tbody>
        ...
        <div className="w-full lg:w-96 space-y-6">
            <div className="bg-gray-850 p-6 rounded-xl border border-gray-750 flex flex-col h-fit">
                <div className="flex items-center justify-between mb-6">
                    ...
                    >
                        {analyzing ? 'Reading Charts...' : 'Scan Patterns'}
                    </button>
                </div>
                ...
                {!token.aiAnalysis ? (
                    <div className="bg-gray-900/50 rounded-lg p-6 text-center border border-dashed border-gray-700">
                        ...
                        {!aiConfig.enabled && <p className="text-red-400 text-xs mt-2">AI is disabled in settings.</p>}
                    </div>
                ) : (
                    <div className="space-y-4 animate-fade-in">
                        <div className={`p-4 rounded-lg border ${token.aiAnalysis.action === 'BUY' ? 'bg-green-900/20 border-green-800' : token.aiAnalysis.action === 'SELL' ? 'bg-red-900/20 border-red-800' : 'bg-gray-800 border-gray-700'}`}>
                            <div className="flex justify-between items-center mb-1">
                                <span className="text-gray-400 text-xs uppercase tracking-wider">Recommendation</span>
                                <span className={`text-lg font-bold ${token.aiAnalysis.action === 'BUY' ? 'text-green-400' : token.aiAnalysis.action === 'SELL' ? 'text-red-400' : 'text-gray-300'}`}>{token.aiAnalysis.action}</span>
                            </div>
